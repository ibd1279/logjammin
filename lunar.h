...
public:
                    ...
                    int nargs=0, int nresults=LUA_MULTRET, int errfunc=0)
    {
        ...
        if (!luaL_checkudata(L, base, T::LUNAR_CLASS_NAME)) {
            ...
            lua_pushfstring(L, "not a valid %s userdata", T::LUNAR_CLASS_NAME);
            return -1;
        }
        ...
        if (lua_isnil(L, -1)) {            // no method?
            lua_settop(L, base-1);           // drop userdata and args
            lua_pushfstring(L, "%s missing method '%s'", T::LUNAR_CLASS_NAME, method);
            return -1;
        }
        ...
        if (status) {
            ...
            lua_pushfstring(L, "%s:%s status = %d\n%s",
                            T::LUNAR_CLASS_NAME, method, status, msg);
    ...
    static int push(lua_State *L, T *obj, bool gc=false) {
        ...
        if (lua_isnil(L, -1)) luaL_error(L, "%s missing metatable", T::LUNAR_CLASS_NAME);
        int mt = lua_gettop(L);
